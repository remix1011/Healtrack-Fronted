...
public class OximetryReportActivity extends AppCompatActivity {
    public static int REQUEST_CODE = 162;
    ...
    private int idUser;
    private List<Oximetry> histories;
    private BarChart chart;

    private FloatingActionButton speedDialFloatingActionButton;
    private String[] sposArr = new String[] {"100%", "99%", "98%", "97%", "96%", "95%", "94%", "93%", "92%", "91%", "90%", "89%", "88%", "87%", "86%", "85%", "84%", "83%", "82%", "81%", "80%", "79%", "78%", "77%", "76%", "75%", "74%", "73%", "72%", "71%", "70%", "69%", "68%", "67%", "66%", "65%", "64%", "63%", "62%", "61%", "60%", "59%", "58%", "57%", "56%", "55%", "54%", "53%", "52%", "51%", "50%"};
    private int[] sposArrInt = new int[] {100, 99, 98, 97, 96, 95, 94, 93, 92, 91, 90, 89, 88, 87, 86, 85, 84, 83, 82, 81, 80, 79, 78, 77, 76, 75, 74, 73, 72, 71, 70, 69, 68, 67, 66, 65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 55, 54, 53, 52, 51, 50 };
    private boolean nuevoAgregado = false;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_oximetry_report);
        hide();
    ...
    private void inicializarDatos() {
        ...
        if (sessionManager.getUser().isSpecialist())
            speedDialFloatingActionButton.setVisibility(View.GONE);
    }
    ...
    private void agregarOximetria() {
        //creo el dialogo
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Seleccione el porcentaje de oxigeno en sangre");

        //agrego la lista
        builder.setItems(sposArr, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int pickedPosition) {
                int spo = sposArrInt[pickedPosition];
                add(spo);
            }
        });
    ...
    private void add(int spo) {
        Oximetry oximetry = new Oximetry();
        oximetry.setSaturation(spo);
        oximetry.setUserId(sessionManager.getiduser());

        progressBar.setVisibility(View.VISIBLE);

        AndroidNetworking.post(HealthTrackApi.OXIMETRIES_URL)
                .addHeaders("Content-Type", "application/json")
                .addJSONObjectBody(oximetry.toJsonObject())
    ...
    private void getHistory() {
        ...
        AndroidNetworking.get(HealthTrackApi.OXIMETRY_HISTORY_BY_USER_IN_DATES(idUser, dayStart, dayEnd))
                .addHeaders("Content-Type", "application/json")
                .setPriority(Priority.HIGH)
                .setTag(getString(R.string.app_name))
                .build()
                .getAsJSONArray(new JSONArrayRequestListener() {
                    ...
                    public void onResponse(JSONArray response) {
                        progressBar.setVisibility(View.GONE);
                        histories.clear();
                        histories.addAll(Oximetry.from(response));
                        if (histories.isEmpty())
                            histories.add(new Oximetry(0, 1, 1, "2022-09-20T09:30:45.163", false));
    ...
    private void cargarGrafico(List<Oximetry> histories) {
        List<String> labels = getXAxisLabels(histories);
        ArrayList<BarEntry> barEntries = new ArrayList<>();
        barEntries = new ArrayList<>();
        for (int i = 0; i < histories.size(); i++) {
            Oximetry history = histories.get(i);
            barEntries.add(new BarEntry(i, (float) history.getSaturation()));
        }

        BarDataSet barDataSet = new BarDataSet(barEntries, "Oxigeno en sangre");
        ...
        barData.setValueFormatter(new ValueFormatter() {
            ...
            public String getBarLabel(BarEntry barEntry) {
                return String.valueOf((int) barEntry.getY()) + " SpO2";
            }
        });
    ...
    private List<String> getXAxisLabels(List<Oximetry> histories) {
        List<String> labels = new ArrayList<>();
        for (Oximetry history : histories) {
