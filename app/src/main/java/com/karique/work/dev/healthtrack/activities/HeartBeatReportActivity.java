...
public class HeartBeatReportActivity extends AppCompatActivity {
    public static int REQUEST_CODE = 159;
    ...
    private int idUser;
    private List<HeartbeatHistory> histories;
    ...
    private void getHistory() {
        ...
        AndroidNetworking.get(HealthTrackApi.HEARTBEAT_HISTORY_BY_USER_IN_DATES(idUser, dayStart, dayEnd))
                ...
                .getAsJSONArray(new JSONArrayRequestListener() {
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        histories.clear();
                        histories.addAll(HeartbeatHistory.from(response));
                        if (histories.isEmpty())
                            histories.add(new HeartbeatHistory(0, 1, 1,"2022-09-20T09:30:45.163", false));
    ...
    private void cargarGrafico(List<HeartbeatHistory> histories) {
        List<String> labels = getXAxisLabels(histories);
        ArrayList<BarEntry> barEntries = new ArrayList<>();
        barEntries = new ArrayList<>();
        for (int i = 0; i < histories.size(); i++) {
            HeartbeatHistory history = histories.get(i);
            barEntries.add(new BarEntry(i, (float) history.getAverageBpm()));
        }

        BarDataSet barDataSet = new BarDataSet(barEntries, "Latidos por minuto promedio");
        ...
        barData.setValueFormatter(new ValueFormatter() {
            ...
            public String getBarLabel(BarEntry barEntry) {
                return String.valueOf((int) barEntry.getY()) + " LPM";
            }
        });
    ...
    private List<String> getXAxisLabels(List<HeartbeatHistory> histories) {
        List<String> labels = new ArrayList<>();
        for (HeartbeatHistory history : histories) {
